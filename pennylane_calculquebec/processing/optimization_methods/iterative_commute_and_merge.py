...
def remove_leaf_zs(tape: QuantumTape, iterations=3) -> QuantumTape:
    ...
    if not hasattr(tape, "operations") or tape.operations is None:
        ...
        raise OptimizationError("iterative_commute_and_merge: Input tape has no operations.")
    new_operations = tape.operations.copy()
    for _ in range(iterations):
        list_copy = new_operations.copy()
        new_operations = []
...
def commute_and_merge(tape) -> QuantumTape:
    ...
    if not isinstance(tape, (QuantumTape, QuantumScript)):
        logger.warning("iterative_commute_and_merge : Input is not a QuantumTape or QuantumScript, returning original tape.")
