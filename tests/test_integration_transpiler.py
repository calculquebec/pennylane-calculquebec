...
class test_integration_transpiler(unittest.TestCase):
    ...
    def test_ghz6(self):
        results={'X90': 21, 'CZ': 11, 'PauliZ': 15, 'PhaseShift': 1}
        
        qnode = qml.QNode(lambda : test_circuits.GHZ(6), self.dev)
        qnode()
        transpiler = Transpiler.get_transpiler(MonarqDefaultConfig(useBenchmark=False))
    ...
    def test_AQFT(self):
        results={'CZ': 279, 'PauliZ': 436, 'PhaseShift': 39, 'X90': 478}
